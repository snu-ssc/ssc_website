...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  return <div className="site-shell">
    ...
    <main>
      ...
      <section id="programs" className="section-v2 programs-v2"><div className="container section-intro" data-reveal><div><p className="kicker">02 / PROGRAMS</p><h2>{copy.programSection.title}</h2></div><p>{copy.programSection.description}</p></div><div className="container"><div className="filter-row" role="tablist" aria-label="프로그램 필터">{copy.categories.map((category) => <button key={category} className={program === category ? "is-active" : ""} role="tab" aria-selected={program === category} onClick={() => setProgram(category)}>{category}</button>)}</div><div className="program-grid">{filteredPrograms.map((item, index) => <article className="program-card" data-reveal key={item.title}><div className={`program-card__visual visual-${index % 4}`}><span>{item.tag}</span><b>{item.index}</b></div><div className="program-card__copy"><p>{item.category}</p><h3>{item.title}</h3><span>{item.description}</span><button aria-label={`${item.title} 자세히 보기`}>자세히 보기 <i>→</i></button></div></article>)}</div></div></section>
      <section id="gallery" className="section-v2 gallery-v2"><div className="container gallery-heading" data-reveal><div><p className="kicker">03 / MOMENTS</p><h2>{copy.gallerySection.title}</h2></div><p>{copy.gallerySection.description}</p></div><div className="container mosaic">{copy.gallery.map((item, index) => <button className={`mosaic__item mosaic__item--${index + 1}`} key={item.title} onClick={() => setGalleryItem(item)} aria-label={`${item.title} 상세 보기`}><Image src={item.image} alt={item.alt} fill sizes="(max-width: 700px) 100vw, 50vw" /><span><small>{item.label}</small><strong>{item.title}</strong><i>↗</i></span></button>)}</div></section>
